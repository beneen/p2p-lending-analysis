...
def map_values_to_int(feature_selected_dataset):
    ...
    feature_selected_dataset['grade'] = feature_selected_dataset['grade'].map(
...
def strip_out_specified_columns(original_dataset_with_loan_status):
    ...
    column_names_to_delete = ["delinq_2yrs", "last_pymnt_d", "chargeoff_within_12_mths", "delinq_amnt", "emp_title",
                              "term",
    ...
    column_names_to_keep = list(
        set(column_names_to_delete).symmetric_difference(set(original_dataset_with_loan_status.columns)))
...
def load_dataset():
    ...
    try:
        ...
        original_uncleaned_dataset = pd.read_csv(
            r'/home/beneen/PycharmProjects/p2p_lending/analysis/data_sample/accepted.csv')
    except:
